...
localized in both vertex domain and frequency domain and can capture local
signal information in a more compact and efficient way.

In this chapter, we propose to use the spectral graph wavelets (SGW),
pioneered by Hammond et al.~\cite{Hammond2011}, for mesh approximation
and compression. To the best of our knowledge, we believe that it is the
...
transform are fundamental tools for compact representations of images,
audio, and video signals. The most prominent applications include the
...
Consider a 3D mesh $M=(V,E)$ with vertices $V$
...
As we mentioned in Chapter~\ref{chap:background}, the Laplacian matrix
of $M$ admits an orthogonal and complete basis of $L^2(M)$, i.e.,
the space of square-integrable functions defined on $M$, and enables
Fourier-like harmonic analysis on the vertex domain.

There are many different formations of discrete Laplacian. A combinatorial
Laplacian matrix is determined solely by the connectivity of the mesh.
A geometric Laplacian, on the other hand, takes into account both the
...
The graph Laplacian admits an eigensystem $\{\lambda_k,\mathbf{\chi}_k\}_{k=0}^{n-1}$,
...
shape analysis, the manifold harmonic basis (MHB).
Fig.~\ref{fig:mhb} visualizes the MHB on an example mesh. It is
straightforward to see that the values of
...
As mentioned earlier, the MHB can be employed to define the graph Fourier transform,
also known as the \emph{manifold harmonic transform} (MHT), which converts
...
human's visual system tends to be more forgiving to the loss of
...
Fig.~\ref{fig:fourier_approx} shows an example of spectral mesh approximation
with different number of MHB.
...
\begin{figure}
 ...
 \begin{subfigure}[b]{0.35\linewidth}
  ...
  \label{fig:fourier4}
 \end{subfigure}
 \caption[Spectral approximation of the wolf model.]
...
\section{Sparse Shape Approximation via SGW}
...
In this section, we propose to use SGW to construct a redundant
...
\label{eq:sa}
\end{equation}
which is called the \emph{Best Basis Selection} problem

If the input signal has $c$ channels, denoted as an $n\times c$ matrix
...
participating atoms for each channel, or enforce to select the same subset
for all the channels and minimize the combined approximation errors. The
...
Eq.~(\ref{eq:simulsa}) is NP-hard with the time complexity of $O(nm^{n'}{n'}^2$,
but we can find approximate solutions using greedy pursuit algorithms such as
...
\subsection{SGW and Dictionary Design Strategies}
...
propose to use normalized multiscale spectral graph wavelets (SGW)
as atoms to construct the dictionary for sparse approximation.

As we mentioned in Chapter~\ref{chap:background}, the spectral graph wavelets (SGW)
are expressed as bivariate kernel functions expanded on the Laplacian eigenbasis
...
scale parameter. The $i$th row of $\Psi_t$

\begin{equation}
\psi_{t,i}(\cdot)=\sum_{k=0}^{n-1} g(t\lambda_k)\chi_k(i)\chi_k(\cdot)
\end{equation}

is the spectral wavelet localized at $v_i$ in the vertex domain and
at scale $t$ in the frequency domain.

The spectral scaling functions is defined with the scaling generator
function $h$ which satisfy
\begin{equation}
  \left\{
  \begin{array}{l}
    h(0)> 0 \\
...
In practice, the scale parameter $t$ also need to be discretized. The spectral graph
wavelets $\psi_{t,i}$ are near orthogonal to $\chi_k$ for $\lambda_k$
near 0, i.e., low-frequency eigenbasis, for any discrete scale $t$. Hence,
to better capture low-frequency signal information, \cite{Hammond2011}
also introduced the \emph{spectral scaling functions} which have similar
constructions with SGW but act like low-pass filters
\begin{equation}
...
\begin{subfigure}[b]{0.3\linewidth}
    ...
    \label{fig:sgw3}
\end{subfigure}%
\caption[Spectral graph wavelets with different scales.]
{Visualizations of spectral graph wavelets localized at the
 same point but with different scales. From left to right, the
 spectral wavelets at scale 1, 3 and 5 are plotted.}
\label{fig:sgw}
\end{figure}

The SGW dictionary has several advantages:
\begin{itemize}
...
The most time-consuming part of S-OMP is the computation of the maximum inner
product between the residual and available atoms, which costs $O(mn)$
in each iteration. If the required number of iterations, i.e., the
sparsity of the solution is $l$, the total time complexity of S-OMP will be $O(lmn)$.
Generally, the required number of iterations $l$ and the size of dictionary $m$ are
linearly proportional to the mesh size $n$, hence the total time
complexity is $O(n^3)$, which is unacceptable for very large meshes.

In addition, all the dictionaries we use are constructed from the eigenvectors of
mesh Laplacian, but the full Laplacian eigendecomposition of a large mesh is
very time consuming and can be numerically instable. Hence, when the input mesh
is very large, it is necessary to perform graph partitioning and perform the
compression algorithm on each individual sub-mesh. As suggested in~\cite{Karni2000},
we use the METIS package~\cite{karypis1998fast} for fast graph partitioning. If
the average size of each submesh is $v$, the time complexity of S-OMP will decrease
to $O(n v^2)$.

As mentioned in Sec.~\ref{sec:compratio}, the overhead ratio
of storing the indices of selected atoms is smaller when the mesh size becomes
larger. Moreover, increasing the number of sub-meshes also increases the
occurrences of unpleasant artifacts along sub-mesh boundaries, known as the edge effects.
On the flip side, partitioning a large mesh to smaller ones may result in better
local fitting.

Taking into account these factors, it is not immediately clear what partition size should be
optimal. In our implementation, a large mesh is decomposed into patches containing
approximately equal number of vertices, with the maximum patch size empirically
set to be $1,000$, which seems to strike a good balance between time cost and approximation error.
Fig.~\ref{fig:partition_perf} shows an example of how the approximation error and encoding time
change with the partition size.

\begin{figure}
    \centering
    \begin{subfigure}{0.28\linewidth}
        \includegraphics[width=\linewidth]{horse_partition}
        \caption{}
    \end{subfigure}
    ~
    \begin{subfigure}{0.33\linewidth}
        \includegraphics[width=\linewidth]{horse_partition_time}
        \caption{}
    \end{subfigure}
    ~
    \begin{subfigure}{0.33\linewidth}
        \includegraphics[width=\linewidth]{horse_partition_error}
        \caption{}
    \end{subfigure}
    \caption[Relationships between approximation performance and
    partition size.]
    {Relationships between approximation performance and
    partition size. (a) The \emph{Horse} model partitioned into 
    patches of about $1,000$ vertices. (b) Time performance \emph{vs} patch size. (c)
    Approximation errors \emph{vs} patch size.}
    \label{fig:partition_perf}
\end{figure}
...
\begin{figure}
    ...
    \begin{subfigure}{0.29\linewidth}
        ...
        \caption{}
    \end{subfigure}
    \caption[Models used in the approximation experiments and their partitionings.]
    {Models used in our approximation experiments and their
     partitionings. (a) \emph{Cow}: 4,315 vertices. (b)
...
%        \caption{}
    ...
    {Comparison of mesh compression performance
        ...
        compression errors change with the target compression ratios.}
    \label{fig:coweval}
\end{figure}
...
\begin{figure}
    ...
    {Comparison of mesh compression performance
        ...
        (e) shows how the compression errors change with the target compression
        ratios.}
...
\hline
Model (\#vertices) & Ratio & Error & Error Reduction & Timing (s) \\
\hline
...
{Statistics of compression errors and
    ...
    The ``Error Reduction'' column denotes how much the compression
    error is reduced comparing   with the MHB truncation method.}
\label{tab:stat}
\end{table}
...
In this chapter, we have developed an algorithm for sparse approximation of 3D
shapes. We employed the spectral graph wavelets to construct the redundant
dictionary of shape bases, and used simultaneous orthogonal matching pursuit
to seek a sparse representation of the input mesh. The use of spatially-localized
...
As for the future work, we plan to investigate other improved formulations of
harmonic basis to enhance the expressive power of dictionary.
For example, it is potentially desirable to have data-dependent, anisotropic
wavelets that are adaptive to shape features such as sharp corners and edges to
attain more efficient and sparse representation of shape geometry. We also
plan to explore faster sparse approximation algorithms such as stagewise orthogonal
matching pursuit (StOMP)~\cite{Donoho2012} to arrive at better time performance. 